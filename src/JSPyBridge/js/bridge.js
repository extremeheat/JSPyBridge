...
class Bridge {
  constructor(ipc) {
    ...
    this.m = {
      0: {
        ...
        flushEventPolling: this.flushEventPolling.bind(this),
  ...
  async startEventPolling(ffid, eventName, pollingId) {
    ...
    const h = pollingId
    if (this.eventMap[h]) {
      const e = this.eventMap[h]
      console.log("Accum", e.accumulated)
      return e.accumulated
    }

    const handler = (...args) => {
      this.eventMap[h]?.accumulated.push(args) ?? what.removeEventListener(eventName, handler)
    }
    what.addEventListener(eventName, handler)
    const accumulated = this.m[++this.ffid] = []
    this.eventMap[h] = { handler, accumulated, id: this.ffid }
  }

  flushEventPolling(pollingId) {
    const h = pollingId
    if (this.eventMap[h]) this.eventMap[h].accumulated = []
  }

  async stopEventPolling(ffid, eventName, pollingId) {
    const what = await this.m[ffid]
    const h = pollingId
    if (this.eventMap[h]) {
      what.removeEventListener(eventName, this.eventMap[h].handler)
      delete this.m[this.eventMap[h].id]
      delete this.eventMap[h]
    }
  }
}
